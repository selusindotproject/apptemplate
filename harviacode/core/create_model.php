...
$string .= "
        ...
        if (\$hakAkses['ubah']) {
            \$action = anchor(site_url('".$c_url."/update/$1'),'Ubah');
            if (\$hakAkses['hapus']) {
                \$action .= \" | \";
            }
        }
        if (\$hakAkses['hapus']) {
            \$action .= anchor(site_url('".$c_url."/delete/$1'),'Hapus','onclick=\"javascript: return confirm(\'Apakah Anda yakin ingin menghapus data ?\')\"');
        }
        \$this->datatables->add_column('action', \$action, '$pk');
